...
* Last Modified by: Sarah D
* Creates table A2 of the PAP of DiD Morocco Pioneer School Project
********************************************************************************
...
if $setglobals==1 {
	...
	// Table
	global tablea2			"${tables}/tablea2.txt" // table2 PAP: Baseline balance for non-attriting students
...
global varlist "gender repeated tayssir theta_arabic theta_french theta_math"
...
global varlistdup "fem rep tay scoa scof scom"

global treat "Untreated Treated"

preserve
keep if (baseline == 1 & attrition == 0)
...
forvalues i = 1/3 {
	...
	if `i' == 1 {
		qui: areg treated theta_arabic female repeated tayssir if subject == `i', absorb(match_pair_grade) vce(cluster pair_id)
		qui test
		local ft_pval`i': di %12.2f `r(p)'
		}
	else if `i' == 2 {
		qui: areg treated theta_french female repeated tayssir if subject == `i', absorb(match_pair_grade) vce(cluster pair_id)
		qui test
		local ft_pval`i': di %12.2f `r(p)'
		}	
	else {
		qui: areg treated theta_math female repeated tayssir if subject == `i', absorb(match_pair_grade) vce(cluster pair_id)
		qui test
		local ft_pval`i': di %12.2f `r(p)'
		}	
...
file open a using "${tablea2}" , write append
