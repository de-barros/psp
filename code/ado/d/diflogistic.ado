...
program diflogistic, rclass sortpreserve
	...
	if missing("`varlist'") {
		...
		local rc = _rc
		return add
		return local _collect_prefix_get
		exit `rc'
	}
	...
	if !missing("`maxp'") {
		...
		if _rc {
			di as err "option {bf:maxp()} must be a number in (0,1)"
			exit 198
		}
		if `maxp' <= 0 | `maxp' >= 1 {
			di as err "option {bf:maxp()} must be in (0,1)"
			exit 198
		}
	}
	...
	if !missing("`items'") {
		...
		if !`in' {
di as err "some items in {bf:items()} not present in {it:varlist}"
exit 198
		}
	}
	...
	tempname full nudif udif ehold
	...
	foreach v of local items {
		...
		// full model
		capture `noisily' logit `v' c.`score'##i.`group' `wgt'
		if _rc == 1 {
			exit 1
		}
		if _rc {
		...
		qui `noisily' logit `v' c.`score' i.`group' `wgt'
		est store `nudif'
		// model for uniform dif
		qui `noisily' logit `v' c.`score' `wgt'
		est store `udif'
	...
	mat coleq `out' = Nonuniform Nonuniform Uniform Uniform
...
program Replay
	...
	if !missing(`"`if'"') {
		di as err "{it:if} not allowed on replay"
		exit 198
	}
	if !missing(`"`in'"') {
		di as err "{bf:in} {it:range} not allowed on replay"
		exit 198
	}
	if !missing(`"`weight'"') {
		di as err "{it:weight} not allowed on replay"
		exit 198
	}
	if !missing(`"`group'"') {
	...
	local items `r(items)'
	if missing("`items'") {
		di as err "macro {bf:r(items)} not found"
		exit 198
	}
	mata: __di_irtdif_logistic()
end
