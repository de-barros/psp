...
{synopt :{opt nolistwise}}do not use listwise deletion to handle missing values{p_end}
{synopt :{opt noyates}}do not apply Yates's correction for continuity; default is to apply the continuity correction{p_end}

{syntab:Reporting}
{synopt :{opt l:evel(#)}}set confidence level; default is {cmd:level(95)}{p_end}
{synopt :{opt maxp(#)}}display only items with {it:p}-value {ul:<} {it:#}{p_end}
{synopt :{opth sf:ormat(%fmt)}}display format for chi-squared values; default is {cmd:sformat(%9.2f)}{p_end}
{synopt :{opth pf:ormat(%fmt)}}display format for {it:p}-values; default is {cmd:pformat(%9.4f)}{p_end}
...
{p 4 6 2}
{cmd:collect} is allowed; see {help prefix}.{p_end}
{marker weight}{...}
...
{phang}
{opt nolistwise} specifies to omit observations where all the variables are
missing.  By default, observations with any missing values are omitted.
...
{phang}
{opt maxp(#)}
	requests that only items with {it:p}-value {ul:<} {it:#} be displayed.
...
{phang}
{opth pformat(%fmt)}
	specifies the display format used for the {it:p}-values
